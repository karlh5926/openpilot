...
def create_es_throttle_control(packer, es_accel_msg, fake_button, cruise_throttle):

  values = copy.copy(es_accel_msg)

  values["Throttle_Cruise"] = cruise_throttle
...
def create_es_rpm(packer, es_rpm_msg, rpm):

  values = copy.copy(es_rpm_msg)
  values["RPM"] = rpm
